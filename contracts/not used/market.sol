...
// Keep track of the length of collections for each address
    ...
    uint256 private constant MIN_PRICE_THRESHOLD = 0.21 ether;  // Set a minimum price threshold for safety
    ...
    bool private sweepFloorEnabled = true;  // Circuit breaker flag
...
function cancelListing(address contractAddress, uint256 tokenId) external onlyTokenOwner(contractAddress, tokenId) {
   ...
   function acceptTradeOffer(address contractAddress1, uint256 tokenId1) external nonReentrant {
    ...
    ERC721 token1 = ERC721(contractAddress1);
    ERC721 token2 = ERC721(offer.contractAddress2);
...
// Batch listing function
    function listTokens(address[] memory contractAddresses, uint256[] memory tokenIds, uint256[] memory prices, uint256[] memory royalties) external payable nonReentrant {
        require(contractAddresses.length == tokenIds.length && tokenIds.length == prices.length && prices.length == royalties.length, "Mismatched input arrays");
    ...
    function processListings(address[] memory contractAddresses, uint256[] memory tokenIds, uint256[] memory prices, uint256[] memory royalties) internal {
